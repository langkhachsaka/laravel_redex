    ...
    render() {
        ...
        const btnUpdate = <button key="btn-update" className="btn btn-sm btn-info square" onClick={() => {
            this.props.actions.openMainModal(<Form model={model}
                                                   setListState={this.props.setListState}/>, "Sửa thông tin công ty chuyển phát");
        }}><i className="ft-edit"/></button>;

        const btnDelete = <button key="btn-delete" className="btn btn-sm btn-danger square" onClick={() => {
            swal({
                title: "Xoá thông tin công ty chuyển phát",
                text: "Bạn có chắc chắn muốn xoá thông tin công ty chuyển phát này?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
        ...
        return (
            [
                userPermissions.courier_company.update && btnUpdate,
                ' ',
                userPermissions.courier_company.delete && btnDelete,
            ]
        );
    }
}
