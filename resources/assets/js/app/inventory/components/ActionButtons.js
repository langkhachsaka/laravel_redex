    ...
    render() {
        ...
        const btnUpdate = <button key="btn-update" className="btn btn-sm btn-info square" onClick={() => {
            this.props.actions.openMainModal(<Form model={model}
                                                   setListState={this.props.setListState}/>, "Sửa thông tin hàng tồn kho");
        }}><i className="ft-edit"/></button>;

        const btnDelete = <button key="btn-delete" className="btn btn-sm btn-danger square" onClick={() => {
            swal({
                title: "Xoá thông tin hàng tồn kho",
                text: "Bạn có chắc chắn muốn xoá thông tin hàng tồn kho này?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
        ...
        return (
            [
                userPermissions.inventory.update && btnUpdate,
                ' ',
                userPermissions.inventory.delete && btnDelete,
            ]
        );
    }
}
