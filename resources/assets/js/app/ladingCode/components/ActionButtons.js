    ...
    render() {
        ...
        const btnUpdate = <button key="btn-update" className="btn btn-sm btn-info square" onClick={() => {
            this.props.actions.openMainModal(<Form model={model}
                                                   setListState={this.props.setListState}/>, "Sửa thông tin nguồn hàng");
        }}><i className="ft-edit"/></button>;

        const btnDelete = <button key="btn-delete" className="btn btn-sm btn-danger square" onClick={() => {
            swal({
                title: "Xoá Nguồn hàng",
                text: "Bạn có chắc chắn muốn xoá nguồn hàng này?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
        ...
        return (
            [
                userPermissions.ladingCode.update && btnUpdate,
                ' ',
                userPermissions.ladingCode.delete && btnDelete
            ]
        );
    }
}
