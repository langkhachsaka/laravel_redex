    ...
    render() {
        ...
        const btnUpdate = <button key="btn-update" className="btn btn-sm btn-info square" onClick={() => {
            this.props.actions.openMainModal(<Form model={model}
                                                   setListState={this.props.setListState}/>, "Sửa thông tin nhân viên " + model.name);
        ...
        const btnDelete = <button key="btn-delete" className="btn btn-sm btn-danger square" onClick={() => {
            swal({
                ...
                text: "Bạn có chắc chắn muốn xoá tài khoản nhân viên này?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            })
        ...
        return (
            [
                userPermissions.user.update && btnUpdate,
                ' ',
                userPermissions.user.delete && btnDelete,
            ]
        );
    }
}
