...
class CreateTaskDeliveryForm extends Component {
    ...
    handleSubmit(formProps) {
        ...
        return ApiService.post(Constant.resourcePath('createTaskDelivery'), {...formProps,transactionDetail : this.state.models})
        .then(({data}) => {
            data.data.map(item => {
                        ...
                        {this.state.models.map((model,index) => {
                            ...
                            return (
                                <div key ={index} className="col-sm-3">
                                    <b>{index + 1 } Khách hàng : {model.customer && model.customer.name}</b><br/>
                                    {model.payment_info && model.payment_info.map((item,index) =>{
                                        let obj = JSON.parse(item.data);
                                        return (
                                            <div key={item.id}>
                                                {item.type == 0 && <span>{obj.address} : {obj.lading_code.toString()}</span> }
                                            </div>
                                        );
                                    }) }
                                </div>)
...
export default connect(null, mapDispatchToProps)(reduxForm({
    form: 'CreateTaskDeliveryForm'
})(CreateTaskDeliveryForm))
